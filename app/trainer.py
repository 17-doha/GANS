...
def report_progress(
    ...
    if eoe and step == (epoch_steps - 1):
        ...
        with torch.no_grad():
            ...
            idx = torch.randint(
                0, X_train.shape[0], (n_samples,), device=device
            )
            X_real = X_train[idx]
        ...
        print(
            f"Discriminator Accuracy on real: {acc_real * 100:.0f}%, "
            f"on fake: {acc_fake * 100:.0f}%"
        )
        ...
        if step % 50 == 0:
            print(
                f"Training progress in epoch #{epoch}, step {step}, "
    ...
    with mlflow.start_run() as run:
        ...
        for epoch in range(epochs):
            for step in range(epoch_steps):
                ...
                # Real data
                idx = torch.randint(
                    0, X_train.shape[0], (half_batch,), device=device
                )
                X_real = X_train[idx]
                ...
                # Use .detach() so we don't backpropagate through
                # the Generator
                d_fake_loss = criterion(
                    discriminator(X_fake.detach()), y_fake
                )
                ...
                # Generator wants the Discriminator to think the fake
                # images are REAL (1)
            ...
            mlflow.log_metric(
                "discriminator_loss", d_loss.item(), step=epoch
            )
            mlflow.log_metric(
                "generator_loss", g_loss.item(), step=epoch
            )
            ...
            if epoch % 10 == 0 or epoch == epochs - 1:
                mlflow.pytorch.log_model(
                    generator, f"generator-epoch-{epoch}"
                )
